...
const LockScreen: React.FC<LockScreenProps> = ({ onUnlock, onFailure, user }) => {
  ...
  const handleNumClick = (num: string) => {
    if (pin.length < 4) {
      ...
      if (newPin.length === 4) {
        // Mock validation - allow "1234" or any 4 digits for demo
        setTimeout(() => {
          if (newPin === '1234') {
             onUnlock('PIN');
             setPin('');
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-slate-900 flex items-center justify-center animate-fade-in backdrop-blur-md">
      <div className="w-full max-w-sm p-8 flex flex-col items-center">
        ...
        {error && <p className="text-red-500 text-sm font-bold mb-4 animate-pulse">Invalid PIN. Try 1234.</p>}
