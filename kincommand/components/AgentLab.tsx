...
import { LedgerEntry, User, FamilySettings } from '../types';
...
interface AgentLabProps {
  ...
  onAddEntries: (newEntries: LedgerEntry[]) => void;
}

type TestStep = 'IDLE' | 'INTEGRITY' | 'STRESS' | 'PRIVACY' | 'FAIRNESS' | 'COMPLETE';

const AgentLab: React.FC<AgentLabProps> = ({ entries, users, settings, onAddEntries }) => {
  const [logs, setLogs] = useState<string[]>(['> KinCommand Test Suite initialized.', '> Auto-start sequence initiated...']);
  ...
  return (
    <div className="space-y-6 animate-fade-in h-[calc(100vh-8rem)] flex flex-col">
      <header className="flex items-center space-x-3 mb-2 flex-shrink-0">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-900">KinCommand Auto-Diagnostics</h1>
